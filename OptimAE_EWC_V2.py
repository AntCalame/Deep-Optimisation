            ...
            if self.model.transited:
                for key in fisher.keys():
                    if key in self.model.ewc_terms.keys():
                        for p in self.model.ewc_terms[key].keys():
                            self.model.ewc_terms[key][p] += fisher[key][p]
                    
                    else:
                        self.model.ewc_terms[key] = fisher[key]   
        
        #update
        elif self.model.mode=="evolution":
            for key in fisher.keys():
                if key in self.model.ewc_terms.keys():
                    for p in self.model.ewc_terms[key].keys():
                        self.model.ewc_terms[key][p] = (self.model.ewc_terms[key][p]*self.model.forgetting_factor
                                                        +fisher[key][p])
