...
train = torch.load("data/exp_transfer.pt")

test = torch.load("data/exp_transfer_test.pt")
SSS=r""
...
class Analysis:
    ...
    def analysis(self):
        ...
        for (a,j),(k,l) in zip(self.train.items(),self.test.items()):
            if a=="0":
                size="100"
            else:
                size="1000"
            ...
            plt.title("Optimisation Comparison - "+size)
            plt.legend()
            plt.savefig(SSS+'\\'+size+'-all.pdf', format='pdf', bbox_inches='tight')
            plt.show()
            ...
            cb.set_ticklabels(np.arange(0, 20, 2))
            cb.set_label("Transition")
            plt.title("Same Search - "+size)
            plt.ylabel("Fitness Ratio")
            plt.xlabel("Transition")
            plt.legend()
            plt.savefig(SSS+'\\'+size+'-same.pdf', format='pdf', bbox_inches='tight')
            plt.show()
            ...
            plt.ylabel("Fitness Ratio")
            plt.xlabel("Transition")
            plt.title("Maximum search - "+size)
            plt.legend()
            plt.savefig(SSS+'\\'+size+'-20.pdf', format='pdf', bbox_inches='tight')
            plt.show()
            
            print("Portability",(l["fitness"]["max"]["t=all"]["global"]["mean"][-1][-1]/
             j["fitness"]["max"]["t=all"]["global"]["mean"][-1]).item())
