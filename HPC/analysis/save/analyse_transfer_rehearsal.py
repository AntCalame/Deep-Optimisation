...
train = torch.load("data/exp5.pt")
test = torch.load("data/exp5_test.pt")
...
class Analysis:
    ...
    def analysis(self):
        ...
        j=self.train[rehearsal]
        l=self.test[rehearsal]
        ...
        plt.title("Optimisation Comparison - "+rehearsal)
        plt.legend()
        plt.savefig(SSS+'\\oc-r-'+rehearsal+'.pdf', format='pdf', bbox_inches='tight')
        plt.show()
        ...
        cb.set_ticklabels(np.arange(0, 20, 2))
        cb.set_label("Transition")
        plt.title("Same Search - "+rehearsal)
        plt.ylabel("Fitness Ratio")
        plt.xlabel("Transition")
        plt.legend()
        plt.savefig(SSS+'\\ss-r-'+rehearsal+'.pdf', format='pdf', bbox_inches='tight')
        plt.show()
        ...
        plt.ylabel("Fitness Ratio")
        plt.xlabel("Transition")
        plt.title("Maximum Search - "+rehearsal)
        plt.legend()
        plt.savefig(SSS+'\\ms-r-'+rehearsal+'.pdf', format='pdf', bbox_inches='tight')
        plt.show()
