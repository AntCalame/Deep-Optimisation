...
class Analysis:
    ...
    def analysis(self):
        ...
        plt.grid()
        plt.title("Comparison of Fitness Ratio (Batch Size)")
        ...
        plt.xticks(torch.arange(1,21))
        plt.grid()
        plt.title("Comparison of Success Rate (Batch Size)")
        ...
        plt.ylabel("Fitness Ratio")
        plt.grid()
        plt.title("Comparison of Fitness Ratio (Batch Size) (zoomed in)")
        plt.tight_layout()
        ...
        plt.ylabel("Success Rate")
        plt.grid()
        plt.title("Comparison of Success Rate (Batch Size) (zoomed in)")
        ...
        plt.grid()
        plt.savefig(SSS+'\\l-d-64.pdf', format='pdf', bbox_inches='tight')
        plt.show()
        ...
        plt.savefig(SSS+'\\l-pp-64.pdf', format='pdf', bbox_inches='tight')
        plt.show()
        ...
        table_do = [[self.dim[i]+"-"+self.tight[j]]+
                     [round((1-d["fitness"]["max"]["t=all"]["macro"]["mean"][i][j][-1].item())*100,3) for n,d in self.D.items()]
                    for i in range(3) for j in range(3)]
        
        table_do += [["total"]+
                     [round((1-d["fitness"]["max"]["t=all"]["global"]["mean"][-1].item())*100,3) for n,d in self.D.items()]]
        
        table = plt.table(
            cellText=table_do,
            colLabels=["Problem Type"]+[d for d in self.D.keys()],
            loc='center',
            cellLoc='center'
        )
