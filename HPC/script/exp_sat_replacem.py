...
path_mkp = os.path.join(path_COProblems, "maxsat")
...
idx_prb = int(np.floor(number_experiment/size_experiment))
...
from COProblems.SAT import SAT
...
nb_item = 100
problem = SAT(idx_prb, device)

method_mode = "replace"
model = DOAE_EWC(problem_size, dropout_prob, device, param, method_mode)
...
for ii in range(nb_iteration):
    ...
    transition_loss = handler.learn_from_population_detail(population, optimizer, l1_coef=l1_coef, batch_size=pop_size)
...
if "l2" in save["transition_loss"].keys():
    save["transition_loss"]["l2"] *= l2_coef
