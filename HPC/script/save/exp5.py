...
param_rehearsal = float(sys.argv[3])
...
from OptimAE_R import ROptimAEHandler
...
problem = MKP("COProblems/mkp/problems"+str(dim)+"d.txt", "COProblems/mkp/fitnesses"+str(dim)+"d.txt", idx_prb, device)

model = DOAE_detail(problem_size, dropout_prob, device)
handler = ROptimAEHandler(model, problem, device)
...
for ii in range(nb_iteration):
    ...
    handler.update_memory(population)
    
    transition_loss = handler.learn_from_population_detail(population, optimizer, l1_coef=l1_coef, batch_size=pop_size, p=param_rehearsal)
