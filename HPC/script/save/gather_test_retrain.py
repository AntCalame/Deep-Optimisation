...
def read(parameters,s):
    ...
    Data = {"fitness" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,5,nb_transit,taille_pop),dtype=torch.float),
            "time" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,5,nb_transit),dtype=torch.float),
            "evaluation" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,5,nb_transit),dtype=torch.float)
            }
    ...
    for i in range(nb_total):
        ...
        for j in range(5):
            ...
            for ss,d in data.items():
                ...
                Data["fitness"][idx_dim][idx_tight][idx_prb][idx_ite][(int(ss)-1)//4]=d["fit"]
                Data["time"][idx_dim][idx_tight][idx_prb][idx_ite][(int(ss)-1)//4]=d["time"]
                Data["evaluation"][idx_dim][idx_tight][idx_prb][idx_ite][(int(ss)-1)//4]=d["eval"]
...
def compute(Data,parameters):
    ...
    f_max_last_macro = f_max_last.reshape((nb_dim,nb_tight,nb_per_type*nb_rep,5))
    ...
    save["evaluation"]["t=-1"]={}
    t_last = Data["time"][:,:,:,:,:,-1]
    save["time"]["t=-1"]["global"]= {"mean" : t_last.reshape((-1,5)).mean(0),
                                     "std" : t_last.reshape((-1,5)).std(0),
                                     "Q1" : torch.quantile(t_last.reshape((-1,5)),0.25,dim=0),
                                     "Q2" : torch.quantile(t_last.reshape((-1,5)),0.5,dim=0),
                                     "Q3" : torch.quantile(t_last.reshape((-1,5)),0.75,dim=0),
                                     "min" : t_last.reshape((-1,5)).min(0).values,
                                     "max" : t_last.reshape((-1,5)).max(0).values,
                                     "distribution": t_last
                                     }
    ...
    save["evaluation"]["t=-1"]["global"]= {"mean" : e_last.reshape((-1,5)).mean(0),
                                     "std" : e_last.reshape((-1,5)).std(0),
                                     "Q1" : torch.quantile(e_last.reshape((-1,5)),0.25,dim=0),
                                     "Q2" : torch.quantile(e_last.reshape((-1,5)),0.5,dim=0),
                                     "Q3" : torch.quantile(e_last.reshape((-1,5)),0.75,dim=0),
                                     "min" : e_last.reshape((-1,5)).min(0).values,
                                     "max" : e_last.reshape((-1,5)).max(0).values,
                                     "distribution": e_last
                                     }
    ...
    t_last_macro = Data["time"][:,:,:,:,:,-1].reshape((nb_dim,nb_tight,nb_per_type*nb_rep,5))
    ...
    e_last_macro = Data["evaluation"][:,:,:,:,:,-1].reshape((nb_dim,nb_tight,nb_per_type*nb_rep,5))
