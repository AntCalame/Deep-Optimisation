...
def read(parameters,s):
    ...
    Data = {"fitness" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit,taille_pop),dtype=torch.float),
            "loss" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit,nb_transit),dtype=torch.float),
            "loss_adapted" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit,nb_transit),dtype=torch.float),
            "correlation" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit,nb_transit),dtype=torch.float),
            "covariance" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit,nb_transit),dtype=torch.float),
            "c_fitness_std" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,1,nb_transit),dtype=torch.float),
            "c_loss_std" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit,nb_transit),dtype=torch.float),
            "dl1" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit,nb_transit),dtype=torch.float),
            "dl1_r" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit,nb_transit),dtype=torch.float),
            "time" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit),dtype=torch.float),
            "evaluation" : torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit),dtype=torch.float)
            }
    ...
    for i in range(nb_total):
        ...
        Data["fitness"][idx_dim][idx_tight][idx_prb][idx_ite] = data["fitness"].detach()
        Data["loss"][idx_dim][idx_tight][idx_prb][idx_ite] = data["loss"].detach()
        Data["loss_adapted"][idx_dim][idx_tight][idx_prb][idx_ite] = data["loss_adapted"].detach()
        Data["correlation"][idx_dim][idx_tight][idx_prb][idx_ite] = data["correlation"].detach()
        Data["covariance"][idx_dim][idx_tight][idx_prb][idx_ite] = data["covariance"].detach()
        Data["c_fitness_std"][idx_dim][idx_tight][idx_prb][idx_ite] = data["c_fitness_std"].detach()
        Data["c_loss_std"][idx_dim][idx_tight][idx_prb][idx_ite] = data["c_loss_std"].detach()
        Data["dl1"][idx_dim][idx_tight][idx_prb][idx_ite] = data["distance_layer1"].detach()
        Data["dl1_r"][idx_dim][idx_tight][idx_prb][idx_ite] = data["distance_layer1_rescaled"].detach()
        Data["time"][idx_dim][idx_tight][idx_prb][idx_ite] = data["time"].detach()
        Data["evaluation"][idx_dim][idx_tight][idx_prb][idx_ite] = data["evaluation"].detach()
        
        for i in data["transition_loss"].keys():
            if "tl_"+i not in Data.keys():
                nb_point = data["transition_loss"][i].size(-1)
                Data["tl_"+i]=torch.zeros((nb_dim,nb_tight,nb_per_type,nb_rep,nb_transit,nb_point),dtype=torch.float)
                Data["tl_"+i][idx_dim][idx_tight][idx_prb][idx_ite]=data["transition_loss"][i]
            else:
                Data["tl_"+i][idx_dim][idx_tight][idx_prb][idx_ite]=data["transition_loss"][i]
        
        if data["sup1"][0].numel()>0:
            if not str(idx_dim) in sup1.keys():
                sup1[str(idx_dim)]={}
                
            if not str(idx_tight) in sup1[str(idx_dim)].keys():
                sup1[str(idx_dim)][str(idx_tight)]={}
                    
            if not str(idx_prb) in sup1[str(idx_dim)][str(idx_tight)].keys():
                sup1[str(idx_dim)][str(idx_tight)][str(idx_prb)]=[]
            
            sup1[str(idx_dim)][str(idx_tight)][str(idx_prb)]+=[data["sup1"]]
...
def compute(Data,parameters):
    ...
    save["fitness"]["max"]["t=-1"]["macro"]= {"mean" : f_max_last_macro.mean(-1),
    ...
    save["fitness"]["max"]["t=-1"]["macro"]["sup"]=(f_max_last_macro>(save["fitness"]["max"]["t=-1"]["macro"]["Q3"]+1.5*save["fitness"]["max"]["t=-1"]["macro"]["IQR"])[...,None]).to(torch.int).mean(-1,dtype=torch.float)
    save["fitness"]["max"]["t=-1"]["macro"]["inf"]=(f_max_last_macro<(save["fitness"]["max"]["t=-1"]["macro"]["Q1"]-1.5*save["fitness"]["max"]["t=-1"]["macro"]["IQR"])[...,None]).to(torch.int).mean(-1,dtype=torch.float)
    ...
    save["fitness"]["mean"]={}
    save["fitness"]["mean"]["global"]={"mean" : f_mean_t.mean((0,1,2,3)),
                                       "std" : f_mean_t.std((0,1,2,3))
                                       }
    save["fitness"]["mean"]["macro"]={"mean" : f_mean_t.mean((2,3)),
                                       "std" : f_mean_t.std((2,3))
                                       }
    save["fitness"]["mean"]["micro"]={"mean" : f_mean_t.mean(3),
                                       "std" : f_mean_t.std(3)
                                       }
    save["time"]={}
    save["evaluation"]={}
    ...
    t_last_macro = Data["time"][:,:,:,:,-1].reshape((nb_dim,nb_tight,nb_per_type*nb_rep))
    save["time"]["t=-1"]["macro"]= {"mean" : t_last_macro.mean(-1),
    ...
    save["time"]["t=-1"]["macro"]["sup"]=(t_last_macro>(save["time"]["t=-1"]["macro"]["Q3"]+1.5*save["time"]["t=-1"]["macro"]["IQR"])[...,None]).to(torch.int).mean(-1,dtype=torch.float)
    save["time"]["t=-1"]["macro"]["inf"]=(t_last_macro<(save["time"]["t=-1"]["macro"]["Q1"]-1.5*save["time"]["t=-1"]["macro"]["IQR"])[...,None]).to(torch.int).mean(-1,dtype=torch.float)
    
    e_last_macro = Data["evaluation"][:,:,:,:,-1].reshape((nb_dim,nb_tight,nb_per_type*nb_rep))
    save["evaluation"]["t=-1"]["macro"]= {"mean" : e_last_macro.mean(-1),
    ...
    save["evaluation"]["t=-1"]["macro"]["sup"]=(e_last_macro>(save["evaluation"]["t=-1"]["macro"]["Q3"]+1.5*save["evaluation"]["t=-1"]["macro"]["IQR"])[...,None]).to(torch.int).mean(-1,dtype=torch.float)
    save["evaluation"]["t=-1"]["macro"]["inf"]=(e_last_macro<(save["evaluation"]["t=-1"]["macro"]["Q1"]-1.5*save["evaluation"]["t=-1"]["macro"]["IQR"])[...,None]).to(torch.int).mean(-1,dtype=torch.float)
    ...
    eval_to_sol_macro = eval_to_sol.reshape((nb_dim,nb_tight,nb_per_type*nb_rep))
    ...
    # mean, std
    # global, macro    
    save["loss"]={"global" : {"mean" : Data["loss"].mean((0,1,2,3)),
                              "std" : Data["loss"].std((0,1,2,3))},
                  "macro" : {"mean" : Data["loss"].mean((2,3)),
                             "std" : Data["loss"].std((2,3))}
                  }
    
    # loss_adapted
    # mean, std
    # global, macro  
    save["loss_adapted"]={"global" : {"mean" : Data["loss_adapted"].mean((0,1,2,3)),
                                      "std" : Data["loss_adapted"].std((0,1,2,3))},
                          "macro" : {"mean" : Data["loss_adapted"].mean((2,3)),
                                     "std" : Data["loss_adapted"].std((2,3))}
                          }
    
    # covariance
    # mean, std
    # global, macro
    save["covariance"]={"global" : {"mean" : Data["covariance"].mean((0,1,2,3)),
                                    "std" : Data["covariance"].std((0,1,2,3))},
                        "macro" : {"mean" : Data["covariance"].mean((2,3)),
                                   "std" : Data["covariance"].std((2,3))}
                        }
    ...
    save["correlation"]={"global" : {"mean" : corr.mean((0,1,2,3)),
                                     "std" : corr.std((0,1,2,3))},
                         "macro" : {"mean" : corr.mean((2,3)),
                                    "std" : corr.std((2,3))}
                         }
    ...
    # mean, std
    # global, macro  
    save["c_fitness_std"]={"global" : {"mean" : Data["c_fitness_std"].mean((0,1,2,3)),
                                       "std" : Data["c_fitness_std"].std((0,1,2,3))},
                           "macro" : {"mean" : Data["c_fitness_std"].mean((2,3)),
                                      "std" : Data["c_fitness_std"].std((2,3))}
                           }
    
    # std loss
    # mean, std
    # global, macro  
    save["c_loss_std"]={"global" : {"mean" : Data["c_loss_std"].mean((0,1,2,3)),
                                    "std" : Data["c_loss_std"].std((0,1,2,3))},
                        "macro" : {"mean" : Data["c_loss_std"].mean((2,3)),
                                   "std" : Data["c_loss_std"].std((2,3))}
                        }
    
    # distance layer1
    # mean, std
    # global, macro  
    save["dl1"]={"global" : {"mean" : Data["dl1"].mean((0,1,2,3)),
                             "std" : Data["dl1"].std((0,1,2,3))},
                 "macro" : {"mean" : Data["dl1"].mean((2,3)),
                            "std" : Data["dl1"].std((2,3))}
                 }
    ...
    for i in Data.keys():
        if i[:3]=="tl_":
            save["transition_loss"][i[3:]]={"global" : Data[i].mean((0,1,2,3)),
                                            "macro" : Data[i].mean((2,3)),
                                            "micro" : Data[i].mean((3))
                                            }
