...
class DOAE_EWC(DOAE_detail):
    ...
    By Antoine Calame
    """
    def __init__(self, input_size: int, dropout_prob: float, device: torch.device, param: float, mode: str):
        """
        ...
        for i in range(size_pop):
            ...
            if i==0:
                ...
                for name, param in self.named_parameters():
                    ...
                    fisher[key][0]["fim"] += (param.grad.data ** 2)/size_pop
    ...
    def loss_ewc(self) -> float:
        ...
        for name,param in self.named_parameters():
            ...
            if key in self.ewc_terms.keys():
                ...
                for ewc_term in  self.ewc_terms[key]:
                    loss = loss + (((param-ewc_term["weight"])**2)
                                   *ewc_term["fim"]).sum()
